...
import AllianceContext from "./components/AllianceContext";

export default function App() {
  const [checkStatus, setCheckStatus] = useState(false);
  const [alliance, setAlliance] = useState(["SA", "OW", "ST"]);

  const allianceContextValue = {
    getAlliance: (allianceInput) => {
      setAlliance(allianceInput);
    },
    alliances: alliance,
    checkbox: () => setCheckStatus(true),
    ...
    <Fragment>
      ...
      <div className="main-content">
        <div className="airline-title">
          <p className="airlines">Airlines</p>
        </div>
        <AllianceContext.Provider value={allianceContextValue}>
          <div className="filter">
            <Filter getAlliance={alliance} />
          </div>
          <div>
